...
const BusinessListingPage = () => {
  ...
  const steps = useMemo<Step[]>(() => {
    ...
    // Only include payment step if payment is not completed
    if (!paymentCompleted) {
  ...
  // Set initial step based on whether payment is completed
  const [currentStep, setCurrentStep] = useState<Step>(paymentCompleted ? 'info' : 'payment');
  ...
  const handlePaymentSuccess = async (paymentData: any) => {
    setLoading(true);
    try {
      // If we're already on the info step (payment was completed externally), just continue
      if (currentStep === 'info') {
        return;
      }
  ...
  const handleNextStep = () => {
    ...
    switch (currentStep) {
      ...
      case 'media':
        ...
        break;
      case 'summary':
        // Submit the business when proceeding from summary
