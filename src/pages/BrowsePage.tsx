...
import { Search, Filter, X, ChevronLeft, ChevronRight, Grid, Crown } from 'lucide-react';
...
const BrowsePage = () => {
  ...
  const initialCategory = categoryParam ? 
    ...
    Object.entries(BusinessCategoryLabels).find(([key, label]) => 
      label.toLowerCase() === decodeURIComponent(categoryParam).toLowerCase() ||
  ...
  const debouncedSearch = useCallback(
    debounce(async (term: string, filters: FilterState, page: number, perPage: number, showAllItems: boolean) => {
      try {
        setLoading(true);
        clearError();
        ...
        console.log('Page:', page, 'Per page:', perPage, 'Show all:', showAllItems);
        ...
        const { data, error } = await supabase.rpc('get_businesses_with_plan_details', {
          p_is_active: true,
          p_search_term: term && term.trim() ? term.trim() : null,
          p_category: filters.category ? BusinessCategoryLabels[filters.category] : null,
          p_location: filters.location && filters.location.trim() ? filters.location.trim() : null,
        ...
        handleError(err, 'Failed to fetch businesses. Please try again.');
        setBusinesses([]);
        setTotalCount(0);
      } finally {
        setLoading(false);
  ...
  }, [searchTerm, filters, currentPage, itemsPerPage, showAll, debouncedSearch]);
  ...
  const getMetaDescription = () => {
    ...
    if (filters.category) {
      description += `Browse Black-owned ${BusinessCategoryLabels[filters.category]} businesses. `;
    }
    ...
    return description + "Connect with talented Black professionals and businesses on BlackOWNDemand.";
  };
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="text-center mb-12">
          ...
          <p className="text-xl text-gray-400">
            Connect with talented Black professionals and businesses across every industry.
          </p>
        </div>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div>
                  ...
                  <Select
                    ...
                    onChange={(options) => handleFilterChange('tags', options.map(opt => opt.value))}
                    options={Object.entries(BusinessTagLabels).map(([value, label]) => ({
                    ...
                    classNames={{
                      ...
                      multiValueRemove: () => "!text-gray-300 hover:!text-white hover:!bg-gray-600",
                      placeholder: () => "!text-gray-400",
                    }}
                  />
                </div>
          ...
          <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
            <div className="text-gray-400">
              {loading ? (
                ...
                <>
                  Showing {showAll ? Math.min(businesses.length, MAX_SHOW_ALL_LIMIT) : Math.min(itemsPerPage, businesses.length)} of {totalCount} businesses
                  {searchTerm && ` for "${searchTerm}"`}
                  {filters.category && ` in ${BusinessCategoryLabels[filters.category]}`}
              ...
              <div className="flex items-center gap-2">
                ...
                <select
                  ...
                  onChange={(e) => {
                    ...
                    handleItemsPerPageChange(value === 'all' ? 'all' : parseInt(value));
                  }}
