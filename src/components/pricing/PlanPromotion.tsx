...
const PlanPromotion: React.FC<PlanPromotionProps> = ({
  planName,
  regularPrice,
  className = "",
}) => {
  ...
  const updateTimeRemaining = () => {
    ...
    if (diffDays > 30) {
      ...
      setTimeRemaining(
        `${diffMonths} month${diffMonths !== 1 ? "s" : ""} left`
      );
    } else {
      ...
      <div className={className}>
        <div className="flex items-baseline">
          <span className="text-5xl font-bold text-white">
            ${(regularPrice / 12).toFixed(0)}
          </span>
          <span className="text-gray-400 ml-2">/month</span>
        </div>
        <p className="text-sm text-gray-500 mt-1">
          (billed annually at ${regularPrice})
        </p>
      </div>
    );
  }
  ...
  return (
    <div className={className}>
      ...
      <div>
        <div className="flex items-baseline">
          <span className="text-5xl font-bold text-white">
            ${(promotion.promotional_price / 12).toFixed(2)}
          </span>
          <span className="text-gray-400 ml-2">/month</span>
        </div>
        <div className="flex items-center mt-1">
          ...
          <p className="text-sm text-gray-500">
            billed annually at ${promotion.promotional_price}
          </p>
        </div>
