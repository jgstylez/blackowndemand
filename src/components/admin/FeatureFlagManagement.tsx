...
const FeatureFlagManagement: React.FC<FeatureFlagManagementProps> = ({ onUpdate }) => {
  ...
  const fetchFeatureFlags = async () => {
    try {
      ...
      handleError(err, 'Failed to load feature flags');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleToggleFlag = async (flagName: string, currentStatus: boolean) => {
    try {
      ...
      const { data, error: toggleError } = await supabase.rpc('toggle_feature_flag', {
        flag_name: flagName,
      ...
      if (data) {
        ...
        setFeatureFlags(prev => 
          prev.map(flag => 
            flag.name === flagName 
              ? { ...flag, is_enabled: !currentStatus, updated_at: new Date().toISOString() } 
              : flag
          )
        );
        
        setSuccess(`Feature flag "${flagName}" ${!currentStatus ? 'enabled' : 'disabled'} successfully`);
      ...
      handleError(err, `Failed to toggle feature flag "${flagName}"`);
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-gray-400">Control which features are enabled in production</p>
        </div>
      ...
      {!isAdmin && (
        <div className="bg-yellow-500/10 border border-yellow-500/20 rounded-lg p-4 text-yellow-400 flex items-start gap-3">
          ...
          <div>
            ...
            <p className="text-sm">
              You have view-only access to feature flags. Only administrators can toggle feature flags.
            </p>
          </div>
        </div>
      )}
      ...
      <div className="bg-blue-500/10 border border-blue-500/20 rounded-lg p-4 text-blue-400 flex items-start gap-3">
        ...
        <div>
          <p className="text-sm">
            Feature flags allow you to control which features are enabled in production without deploying new code.
            Toggle features on or off to control their visibility across the platform.
          </p>
        </div>
      </div>
            ...
            {featureFlags.map(flag => (
              <div key={flag.id} className="bg-gray-900 rounded-xl p-6">
                <div className="flex items-center justify-between">
                  ...
                  <div>
                    <button
                      onClick={() => handleToggleFlag(flag.name, flag.is_enabled)}
                      disabled={toggleLoading === flag.name || !isAdmin}
                      className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
                        flag.is_enabled ? 'bg-green-500' : 'bg-gray-700'
                      } ${(toggleLoading === flag.name || !isAdmin) ? 'opacity-50 cursor-not-allowed' : ''}`}
                      aria-label={flag.is_enabled ? 'Disable feature' : 'Enable feature'}
                      title={!isAdmin ? 'Only administrators can toggle feature flags' : undefined}
                    >
                      <span
