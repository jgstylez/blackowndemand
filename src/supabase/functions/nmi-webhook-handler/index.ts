...
function verifyWebhookSignature(payload: string, signature: string): boolean {
  ...
  if (!NMI_WEBHOOK_SECRET) {
    console.warn('NMI_WEBHOOK_SECRET not configured, skipping webhook signature verification');
    return true;
  }
  ...
  try {
    ...
    switch (eventType) {
      case 'recurring_payment_success':
        await handleRecurringPaymentSuccess(subscriptionId, transactionId, amount);
        break;
        
      case 'recurring_payment_failed':
        await handleRecurringPaymentFailed(subscriptionId, transactionId, amount, webhookData.response_text || '');
        break;
...
async function handleRecurringPaymentSuccess(subscriptionId: string, transactionId: string, amount: number) {
  console.log(`Processing successful recurring payment: ${subscriptionId}, ${transactionId}, $${amount}`);
  ...
  try {
    ...
    const { error: updateError } = await supabase
      ...
      .update({
        ...
        next_billing_date: new Date(Date.now() + 365 * 24 * 60 * 60 * 1000).toISOString() // 1 year from now
      })
      .eq('id', business.id);
      
    if (updateError) {
      console.error('Error updating business after successful payment:', updateError);
    }
    ...
    console.log(`Successfully processed recurring payment for business ${business.id}`);
  } catch (error) {
...
async function handleRecurringPaymentFailed(subscriptionId: string, transactionId: string, amount: number, responseText: string) {
  console.log(`Processing failed recurring payment: ${subscriptionId}, ${transactionId}, $${amount}`);
  ...
  try {
    ...
    if (updateError) {
      console.error('Error updating business after failed payment:', updateError);
    }
    ...
    console.log(`Processed failed recurring payment for business ${business.id}`);
...
async function handleSubscriptionCancelled(subscriptionId: string) {
  ...
  try {
    ...
    if (updateError) {
      console.error('Error updating business after subscription cancellation:', updateError);
    }
    
    console.log(`Successfully processed subscription cancellation for business ${business.id}`);
...
async function handleSubscriptionUpdated(subscriptionId: string) {
  ...
  try {
    ...
    console.log(`Successfully processed subscription update for business ${business.id}`);
