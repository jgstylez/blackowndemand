  ...
  try {
    ...
    try {
      await sendPaymentConfirmationEmail(
        business.email,
        amount,
        `Annual ${business.plan_name || "Business Listing"} Renewal`,
        business.plan_name || "Business Listing",
        transactionId,
        undefined, // We don't have payment method details in webhook
        nextBillingDate
      );
      console.log(
        `Payment confirmation email sent for recurring payment to ${business.email}`
      );
    } catch (emailError) {
      console.error(
