...
function getNMIErrorMessage(responseCode: string, responseText: string): string {
  ...
  return errorMessages[responseCode] || responseText || 'Payment processing failed. Please try again.';
}
  ...
  try {
    ...
    console.log('Request body received:', JSON.stringify({
      ...
      payment_method: requestBody.payment_method ? {
        ...
        card_number: requestBody.payment_method.card_number ? `****${requestBody.payment_method.card_number.slice(-4)}` : 'Missing',
    ...
    if (!business_id || !payment_method || !payment_method.card_number || !payment_method.expiry_date || !payment_method.cvv) {
      return new Response(
    ...
    if (!business.nmi_subscription_id || !business.nmi_customer_vault_id) {
      return new Response(
        JSON.stringify({ error: 'No active subscription found for this business' }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (!parsedResponse.success) {
      const errorMessage = getNMIErrorMessage(parsedResponse.responseCode, parsedResponse.responseText);
      
      return new Response(
        JSON.stringify({ 
          error: errorMessage,
          code: parsedResponse.responseCode || 'unknown_error',
          details: parsedResponse.responseText || 'No additional details available'
        }),
        { status: 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
      );
    }
    ...
    if (updateError) {
      console.error('Error updating business payment method details:', updateError);
    }
    ...
    if (historyError) {
      console.error('Error logging payment method update history:', historyError);
    }
    ...
    return new Response(
      JSON.stringify({ 
        error: 'An unexpected error occurred while updating your payment method. Please try again or contact support.',
