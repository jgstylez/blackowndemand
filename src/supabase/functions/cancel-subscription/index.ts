...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!business_id) {
      return new Response(
        JSON.stringify({ error: 'Missing business ID' }),
    ...
    if (historyError) {
      console.error('Error logging subscription cancellation history:', historyError);
    }
    ...
    return new Response(
      JSON.stringify({ 
        error: 'An unexpected error occurred while cancelling your subscription. Please try again or contact support.',
